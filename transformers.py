...
def prepocess_input_sequence(
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    for input_string in input_str.split():
        if input_string not in token_dict.keys():
            for num in list(input_string):
                out.append(token_dict.get(num))
        else:
            out.append(token_dict.get(input_string))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    mat_mul = torch.zeros((K, K))
    for i in range(K):
          for j in range(K):
            mat_mul[i, j] = query[i, :].dot(key[j, :]) / M**0.5
    mat_mul = F.softmax(mat_mul, dim = 1)
    out = mat_mul.mm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    mat_mul = torch.zeros(N, K, K)
    for i in range(K):
        for j in range(K):
            bmm_out = query[:, i].reshape(N, 1, -1).bmm(key[:, j].reshape(N, -1, 1))/M**0.5
            mat_mul[:, i, j] = bmm_out.reshape(-1)
    mat_mul = F.softmax(mat_mul, dim = 2)  
    out = mat_mul.bmm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    product = query.bmm(key.permute(0, 2, 1)) #(N,K,M)*(N,M,K)=(N,K,K)
    scaled_product = product / M**0.5 #(N,K,K)
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # masl_mtx = torch.masked_fill(mask, -1e9) #(N,K,K)
        masl_mtx = torch.ones(mask.size(), device = query.device)*-1e9*mask
        scaled_product += masl_mtx #(N,K,K)
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(scaled_product, dim=2, dtype=query.dtype) #(N,K,K)
    y = weights_softmax.bmm(value) #(N,K,K)*(N,K,M)=(N,K,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.q.weight, -(6 / (dim_in + dim_q))**0.5, (6 / (dim_in + dim_q))**0.5)
        self.k = nn.Linear(dim_in, dim_q)
        nn.init.uniform_(self.k.weight, -(6 / (dim_in + dim_q))**0.5, (6 / (dim_in + dim_q))**0.5)
        self.v = nn.Linear(dim_in, dim_v)
        nn.init.uniform_(self.v.weight, -(6 / (dim_in + dim_v))**0.5, (6 / (dim_in + dim_v))**0.5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.single_head_lists = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for i in range(num_heads)])
        self.linear = nn.Linear(dim_out*num_heads, dim_in)
        nn.init.uniform_(self.linear.weight, -(6 / (dim_in + dim_out*num_heads))**0.5, (6 / (dim_in + dim_out*num_heads))**0.5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        y_list = []
        for single_head_list in self.single_head_lists:
            y_list.append(single_head_list(query, key, value, mask))
        y_selfatt = torch.cat(y_list, dim = 2)
        y = self.linear(y_selfatt)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.scale_para = nn.Parameter(torch.ones(emb_dim))
        self.shift_para = nn.Parameter(torch.zeros(emb_dim))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x: Tensor):
        ...
        x_std = (torch.sum((x - x_mean)**2, dim=-1)/x.shape[-1])**0.5
        if len(x.size())==2:
          x_std = x_std.reshape(x.shape[0], -1)
        else:
          x_std = x_std.reshape(x.shape[0], -1, 1)
        y = self.scale_para * (x - x_mean) / (x_std + self.epsilon) + self.shift_para
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        args:
            inp_dim: int representing embedding dimension of the input tensor #M
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.MLP_1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        nn.init.uniform_(self.MLP_1.weight, -(6 / (inp_dim + hidden_dim_feedforward))**0.5, (6 / (inp_dim + hidden_dim_feedforward))**0.5)
        self.MLP_2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(self.MLP_2.weight, -(6 / (inp_dim + hidden_dim_feedforward))**0.5, (6 / (inp_dim + hidden_dim_feedforward))**0.5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        y = self.MLP_2(F.relu(self.MLP_1(x)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.layer_norm = LayerNormalization(emb_dim = emb_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.multi_head(query = x, key = x, value = x)
        out1_2 = self.layer_norm(out1 + x)
        out2 = self.dropout(out1_2)
        out3 = self.feed_forward(out2)
        y = self.dropout(self.layer_norm(out2 + out3))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.size()
    mask = torch.triu(torch.ones((K, K), dtype=seq.dtype, device = seq.device), diagonal=1)
    mask = mask.unsqueeze(0).expand(N, -1, -1).bool()
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    K_tensor = torch.arange(start=0, end=1, step=1/K)
    y = torch.zeros(1,K,M)
    for i in range(M):
        y[:,:,i] = K_tensor
    ##############################################################################
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    y = torch.zeros(1,K,M)
    for i in range(M):
        a_pow = torch.div(i//2, M, rounding_mode='floor')
        dem = 10000**a_pow
        if i%2 == 0:
            y[:,:,i] = torch.sin(p/dem)
        else:
